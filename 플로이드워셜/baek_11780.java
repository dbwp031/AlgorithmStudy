            ...
            for (int s = 1; s < n + 1; s++) {
                for (int t = 1; t < n + 1; t++) {
                    if (dist[s][t] > dist[s][i] + dist[i][t]) {
                        ...
                        combinedList.addAll(move[s][i]);
                        combinedList.addAll(move[i][t].subList(1, move[i][t].size()));
                        move[s][t] = combinedList;
                    }
                }
            }
        }
        ...
        for (int i = 1; i < n + 1; i++) {
            for (int j = 1; j < n + 1; j++) {
                if(i==j) System.out.println(0);
                    ...
                    StringBuilder sb = new StringBuilder();
                    sb.append(move[i][j].size() + " ");
                    for (Integer m : move[i][j]) {
                        sb.append(m + " ");
                    }
                    System.out.println(sb);
                }
